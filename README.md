...
- кнопка **«Добавить мод»** в браузере: вставьте ссылку (вложение форума или прямой `.zip`) и установите, либо импортируйте локальный файл `.zip`; любой импорт идёт через общий конвейер — стейджинг `.part`, структурная проверка zip (лимиты записей/распакованного объёма), no-clobber; локальный импорт в ledger не пишется (происхождение файла местное);
