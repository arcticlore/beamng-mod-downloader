...
- пять источников модов:
  ...
  - **Форум BeamNG** — поиск требует входа в браузере, поэтому источник выключен по умолчанию; установка работает по канонической ссылке на вложение `…/attachments/<id>/` (кнопка «Добавить мод»): приложение само распознаёт ссылку, скачивает и проверяет архив;
  - **Прямая ссылка** — произвольный `.zip` по прямой ссылке (http/https) с allowlist-доменами и SSRF-гейтом; тоже выключен по умолчанию и включается там же, в «Добавить мод»;
- кнопка **«Добавить мод»** в браузере: вставьте ссылку (вложение форума или прямой `.zip`) и установите, либо импортируйте локальный файл `.zip`; любой импорт идёт через общий конвейер — стейджинг `.part`, структурная проверка zip (лимиты записей/распакованного объёма), no-clobber; локальный импорт в ledger не пишется (происхождение файла местное) и в проверке обновлений не участвует;
...
src/picker.ts               динамический импорт tauri-plugin-dialog (файловый выбор .zip)
src/components/             ModCard, ModsBrowser, DetailModal, InstalledPanel, SettingsModal (включая секцию «Источники»), LinkImportModal, ProgressBar
...
src-tauri/src/download.rs   менеджер загрузок с прогрессом (стейджинг + лимиты + no-clobber)
src-tauri/src/installer.rs  список/удаление установленных модов
src-tauri/src/archive.rs    структурная проверка zip (magic, central directory, лимиты записей и объёма, unsafe paths)
src-tauri/src/sources/      registry.rs (единый список источников) + worldofmods.rs, beamngweb.rs, github.rs, beamngforum.rs, directurl.rs, probe.rs (+ mod.rs, SourceError)
```
...
- `enabled_sources` — массив id источников, которым разрешены сетевые запросы.
  ...
  - у **существующего** конфига от версии ≤ 0.2.x включаются все три источника,
    ...
    0.2.x сохраняется.
  Новые источники (`beamngforum`, `directurl`) выключены по умолчанию и не
  добавляются к старым конфигам: установка по ссылке из «Добавить мод»
  потребует включить нужный источник. Существующие конфиги от 0.3.x меняются
  только вручную.
